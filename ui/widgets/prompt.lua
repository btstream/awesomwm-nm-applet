...
local function prompt_text_with_cursor(args)
    ...
    elseif #text < args.cursor_pos then
        ...
        char = gstring.xml_escape(
            text:sub(args.cursor_pos, args.cursor_pos + offset)
        )
        spacer = " "
...
function prompt.run(
    ...
    local inv_col = args.fg_cursor
        or theme.prompt_fg_cursor
        or theme.fg_focus
        or "black"
    local cur_col = args.bg_cursor
        or theme.prompt_bg_cursor
        or theme.bg_focus
        or "white"
    ...
    grabber = keygrabber.run(function(modifiers, key, event)
        ...
        if keypressed_callback then
            ...
            if new_command or new_prompt then
                if new_command then command = new_command end
                if new_prompt then prettyprompt = new_prompt end
                update()
            end
            if user_catched then
                if changed_callback then changed_callback(command) end
                return
            end
        end
            ...
            for _, v in ipairs(hooks[key]) do
                if #filtered_modifiers == #v[1] then
                    ...
                    if match then
                        ...
                        if cb then exec(cb, original_command) end
        ...
        -- Get out cases
        if
            (mod.Control and (key == "c" or key == "g"))
            or (not mod.Control and key == "Escape")
        then
            keygrabber.stop(grabber)
            textbox:set_markup("")
        ...
        if mod.Control then
            ...
            elseif key == "r" then
                ...
                for i, v in
                    function(a, i) return itera(-1, a, i) end,
                    data.history[history_path].table,
                    history_index
                do
            ...
            elseif key == "h" then
                if cur_pos > 1 then
                    ...
                    command = command:sub(1, cur_pos - 2 - offset)
                        .. command:sub(cur_pos)
            ...
            elseif key == "w" or key == "BackSpace" then
                ...
                command = command:sub(1, cword_start_pos - 1)
                    .. command:sub(cword_end_pos + 1)
        ...
        elseif mod.Mod1 or mod.Mod3 then
            ...
            elseif key == "d" then
                command = command:sub(1, cur_pos - 1)
                    .. command:sub(cword_end(command, cur_pos))
            ...
            if completion_callback then
                if key == "Tab" or key == "ISO_Left_Tab" then
                    ...
                    local matches
                    command, cur_pos, matches = completion_callback(
                        command_before_comp,
                        cur_pos_before_comp,
                        ncomp
                    )
...
local function spawn_and_handle_error(self, ...)
    ...
    local result = f(...)
    if type(result) == "string" then self.widget:set_text(result) end
end
...
function widgetprompt.new(args)
    ...
    promptbox.password = args.password
    promptbox.run = run
    ...
    promptbox.exe_callback = args.exe_callback
        or function(...) promptbox:spawn_and_handle_error(...) end
...
function widgetprompt.mt:__call(...) return widgetprompt.new(...) end
