...
function overflow:fit(context, orig_width, orig_height)
    ...
    if num_widgets < 1 then return 0, 0 end
    ...
    if need_scrollbar then used_max = used_max + scrollbar_width end
...
function overflow:layout(context, orig_width, orig_height)
    ...
    for i, w in ipairs(widgets) do
        ...
        if scrolled_pos > avail_in_dir then break end

        if is_y then
            content_x, content_y = widget_x, scrolled_pos
            pos = pos + content_h + spacing

            if self._private.fill_space then content_w = width end
        else
            content_x, content_y = scrolled_pos, widget_y
            pos = pos + content_w + spacing

            if self._private.fill_space then content_h = height end
        end
...
function overflow:get_scroll_factor() return self._private.scroll_factor end
...
function overflow:set_scrollbar_width(width)
    if self._private.scrollbar_width == width then return end
...
function overflow:set_scrollbar_position(position)
    if self._private.scrollbar_position == position then return end
...
function overflow:set_scrollbar_enabled(enabled)
    if self._private.scrollbar_enabled == enabled then return end
...
function overflow:get_scrollbar_enabled() return self._private.scrollbar_enabled end
...
function overflow:get_scrollbar_widget() return self._private.scrollbar_widget end
...
function overflow.horizontal(...) return new("horizontal", ...) end
...
-- @tparam widget ... Widgets that should be added to the layout.
-- @constructorfct wibox.layout.overflow.horizontal
function overflow.vertical(...) return new("vertical", ...) end
